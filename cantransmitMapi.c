...
int main(int argc, char **argv)
{
	...
	*/
	...
	fprintf(stdout, "Appuyer sur 'm' pour envoyer Mapi\r\n");
	ToucheLue = getchar();
	...
	while (ToucheLue != 'q')
	{
		if(ToucheLue == 'm')
		{
	   		 fprintf(stdout,"Touche m appuyer\r\n");
			...
			*/
			frame.can_id = 0x666;  	// identifiant CAN, exemple: 247 = 0x0F7
			frame.can_dlc = 4;		// nombre d'octets de données
			sprintf(frame.data, "Mapi");  // données 
